...
(async () => {
  ...
  await check('calculate-price retirement remains complete through the policy-readiness cache refresh', async () => {
    ...
    assert.match(cacheName || '', /^linkmia-v1\.3\.\d+$/, 'a static cache rung is declared (the ladder itself is checked once, in pending-edit-hydration)');
    ...
    assert.match(runtimeName || '', /^linkmia-runtime-v\d+$/, 'a runtime cache rung is declared; both move together (checked in pending-edit-hydration)');
