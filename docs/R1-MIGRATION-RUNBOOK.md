...
STATUS (corrected 2026-09-08): migration 018 is INSTALLED in production
(executed 2026-09-01, checksum-matched, post-install grid + live smoke
verified). `/api/quote-ride` is ENABLED (`QUOTE_SERVICE_DISABLED=0`; the
variable is kept as the one-edit emergency stop), `pricing_state` is
`observe`, and the browser-flag activation release is MERGED AND LIVE
(PR #88, 2026-09-04) per docs/BROWSER-FLAG-ACTIVATION.md. The
...
1. Confirm the reviewed commit; re-run the full suite (three timezones),
   ...
   the 017 bodies WITH the PR-T guard, byte-exact to the guard transform), and `node tests/r1-migration-chain.test.js`
...
3. SQL SECOND: paste the checksum-matched `018_r1_rollback.sql`. Since PR-T
   it restores the 017 bodies WITH the pickup guard (generated by
   `database/migrations/tools/prt-guard-transform.js`, byte-exact to that
   transform, test-asserted) and is SELF-CONTAINED: it creates the
   `linkmia_pickup_is_future` helper first, so it is valid whether or not
   migration 019 was ever installed. It ends with its own verification DO
