...
const dim = (m: SystemModel, id: string, now = T2) =>
  computeSignature(evaluateSystem(m), { id: "tmp", now, mode: "current", subjectId: "ren" }).dimensions.find((d) => d.dimensionId === id)!;
...
describe("fictional user: from incomplete observations to a compared history", () => {
  it("walks the loop end to end", async () => {
    ...
    const first = computeSignature(evaluateSystem(m), { id: "sig_first", now: T1, mode: "current", subjectId: "ren" });
    ...
    m = M.addVariable(m, { id: INPUT_IDS.liquidReserves, subjectId: "sys_ren", name: "Liquid reserves", category: "buffer", changeSpeed: "slow", unit: "$", sourceType: "measured", confidence: 0.95, currentValue: 1500, referenceRange: { min: 0, max: 20000 }, evidence: [{ text: "Savings balance 2026-01-10", sourceType: "measured" }] });
    ...
    const second = computeSignature(evaluateSystem(m), { id: "sig_second", now: T1, mode: "current", subjectId: "ren" });
    ...
    m = M.addSignatureSnapshot(m, computeSignature(evaluateSystem(m), { id: M.nextSignatureId(m), now: T1, mode: "current", subjectId: "ren", label: "Ren household 2026-01" }));
    m = await svc.save(m);
    ...
    m = M.addVariable(m, { id: INPUT_IDS.protectedHours, subjectId: "ren", name: "Protected hours", category: "asset", changeSpeed: "slow", unit: "hours/week", sourceType: "self_reported", confidence: 0.5, currentValue: 3, referenceRange: { min: 0, max: 20 } });
    const personA = M.addRelationship(m, { kind: "causal_hypothesis", participatesInDynamics: true, sourceVariableId: "financial_pressure", targetVariableId: INPUT_IDS.protectedHours, direction: "positive", strength: 0.7, lag: { value: 2, unit: "weeks" }, confidence: 0.5, sourceType: "observed", explanation: "Pressure increases work effort." });
    const personB = M.addRelationship(m, { kind: "causal_hypothesis", participatesInDynamics: true, sourceVariableId: "financial_pressure", targetVariableId: INPUT_IDS.protectedHours, direction: "negative", strength: 0.7, lag: { value: 2, unit: "weeks" }, confidence: 0.5, sourceType: "observed", explanation: "Pressure leads to avoidance." });
    const sigA = computeSignature(evaluateSystem(personA), { id: "a", now: T2, mode: "current", subjectId: "ren" });
    const sigB = computeSignature(evaluateSystem(personB), { id: "b", now: T2, mode: "current", subjectId: "ren" });
