...
-- accept_quote_create and accept_quote_edit BYTE-EXACTLY (extracted
-- programmatically from 017; tests/r1-route-content.test.js asserts the
-- match). Signatures identical, so ACLs/ownership/SECURITY DEFINER survive.
...
-- known, reviewed price of rollback; mode stays 'off' so nothing verified
-- flows anyway.
-- ============================================================

BEGIN;

-- Pre-capture (round-2 P1): the restore must not launder privilege or
-- identity drift into a committed rollback. Anything already wrong aborts
-- HERE, before the bodies move.
...
BEGIN
  IF (SELECT count(*) FROM r1_rb_pre_state) <> 2 THEN
    RAISE EXCEPTION 'R1 rollback: expected exactly the two public writers';
  END IF;
  FOR v_row IN SELECT * FROM r1_rb_pre_state LOOP
    IF has_function_privilege('anon', v_row.oid, 'EXECUTE')
      ...
      RAISE EXCEPTION 'R1 rollback: client role holds EXECUTE on % — refuse to roll back over privilege drift', v_row.proname;
    END IF;
    IF NOT has_function_privilege('service_role', v_row.oid, 'EXECUTE') THEN
      RAISE EXCEPTION 'R1 rollback: service_role lacks EXECUTE on % — refuse to roll back over privilege drift', v_row.proname;
    END IF;
  END LOOP;
END;
$r1_rb_precheck$;
...
-- Post-restore verification: the 017 semantics must PROVABLY be back, and
-- identity/owner/ACL/config must equal the pre-capture, with the privilege
-- ceiling re-proven behaviorally.
DO $r1_rb_verify$
DECLARE v_row RECORD; v_def TEXT;
BEGIN
  FOR v_row IN SELECT * FROM r1_rb_pre_state LOOP
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'R1 rollback: identity/owner/ACL/config drifted for % during restore', v_row.proname;
    END IF;
    IF NOT (SELECT prosecdef FROM pg_proc WHERE oid = v_row.oid) THEN
      RAISE EXCEPTION 'R1 rollback: SECURITY DEFINER missing on %', v_row.proname;
    END IF;
    IF NOT has_function_privilege('service_role', v_row.oid, 'EXECUTE') THEN
    ...
    IF has_function_privilege('anon', v_row.oid, 'EXECUTE')
       OR has_function_privilege('authenticated', v_row.oid, 'EXECUTE') THEN
      RAISE EXCEPTION 'R1 rollback: client role holds EXECUTE on %', v_row.proname;
    END IF;
  ...
  IF (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    ...
    RAISE EXCEPTION 'R1 rollback: overload detected';
  END IF;
END;
$r1_rb_verify$;
