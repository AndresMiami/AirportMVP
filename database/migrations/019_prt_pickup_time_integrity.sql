-- ============================================================
-- Migration 019 - PR-T: pickup-time integrity (plan v8.6 section 3E)
--
...
BEGIN
  ...
  IF v_create_fp IS NULL OR v_edit_fp IS NULL
     OR NOT ((v_create_fp, v_edit_fp) IN (('ed86cca5e4f5046dc9503771a38bb63f7b4140956dfb94b645f076cd59483388', 'cc56cc673236db967d738209c9f9d5423e0e7117a74358e48280c08f0cd14b41'), ('3bb8663d8a052c02512538f3beb147740ae956e41c5f757a01c85b65c7cb85b6', '07fdcd2d564c6fbdf043b0b0d6f17ff67c8b3f3736153217e034a329822c1c81'))) THEN
      RAISE EXCEPTION 'PR-T: the installed writer pair is not a recognized reviewed pair (accept_quote_create body fingerprint %, accept_quote_edit body fingerprint %; accepted pairs: reviewed_018 (ed86cca5e4f5.., cc56cc673236..); reviewed_018_as_pasted_2026_09_01 (3bb8663d8a05.., 07fdcd2d564c..)) - an unreviewed or mixed state is installed; refusing to overwrite it',
...
BEGIN
  ...
  FOR v_row IN SELECT * FROM prt_pre_state LOOP
    ...
    IF encode(extensions.digest((SELECT prosrc FROM pg_proc WHERE oid = v_row.oid), 'sha256'), 'hex')
       <> (CASE v_row.proname WHEN 'accept_quote_create' THEN 'bed377560252619862bcad768a2d5615496aca0f3d5016c4731d5afad651b6e1' ELSE '0acb70791a784c9569ab062dba2c04e17be380f9e0a9ed28361f1f5dea2d5ed8' END) THEN
