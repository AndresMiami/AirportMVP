...
BEGIN
  ...
  IF p_verdict = 'verified' AND v_effective_verdict <> 'verified' THEN
    INSERT INTO quote_verifications (verdict, mode, purpose, identity_hash)
      VALUES (v_effective_verdict, v_mode, 'create', v_idhash);
  ...
  ELSIF v_mode = 'observe' THEN
    IF p_client_price IS NULL OR p_client_price::text IN ('NaN','Infinity','-Infinity')
       OR p_client_price < 0 OR p_client_price > 21474836.47 THEN
  ...
  BEGIN
    INSERT INTO bookings (
      ...
      -- Google route facts are not persisted before policy clearance. A
      -- verified create therefore stores no duration; legacy/unverified
      -- traffic retains today's client-provided duration.
      CASE WHEN p_verdict = 'verified' THEN NULL
        ELSE NULLIF(p_booking->>'duration_minutes','')::INTEGER END,
      'pending', COALESCE(p_booking->>'source','website')
    ) RETURNING id INTO v_booking_id;

    IF v_effective_verdict = 'verified' AND v_mode <> 'off' THEN
...
BEGIN
  ...
  IF p_verdict = 'verified' AND (
    ...
    OR (p_payload->>'authUserId')::UUID IS DISTINCT FROM p_auth_user_id
    OR (p_payload->>'customerId')::UUID IS DISTINCT FROM p_customer_id
    OR (p_payload->>'bookingId')::UUID IS DISTINCT FROM p_booking_id
  ) THEN
  ...
  IF p_verdict = 'verified' AND v_effective_verdict <> 'verified' THEN
    INSERT INTO quote_verifications (verdict, mode, purpose, identity_hash)
      VALUES (v_effective_verdict, v_mode, 'edit', v_idhash);
    RETURN jsonb_build_object('outcome', CASE
      WHEN v_effective_verdict = 'expired' THEN 'quote_expired'
      ELSE 'quote_not_yet_valid' END);
  END IF;
  ...
  IF p_verdict = 'verified' THEN
    ...
    IF (p_payload->>'iat')::BIGINT > v_now_ms + 60000 THEN
      INSERT INTO quote_verifications (verdict, mode, purpose, booking_id, identity_hash)
        VALUES ('not_yet_valid', v_mode, 'edit', p_booking_id, v_idhash);
      RETURN jsonb_build_object('outcome', 'quote_not_yet_valid');
    ELSIF v_now_ms >= (p_payload->>'exp')::BIGINT THEN
      INSERT INTO quote_verifications (verdict, mode, purpose, booking_id, identity_hash)
        VALUES ('expired', v_mode, 'edit', p_booking_id, v_idhash);
  ...
  BEGIN
    UPDATE bookings SET
      ...
      -- A cryptographically verified replacement route cannot retain the old
      -- route's duration.  Raw Google route facts are not persisted before
      -- policy clearance, so clear it rather than storing or guessing.
      duration_minutes = CASE WHEN p_verdict = 'verified' THEN NULL
        ELSE COALESCE((p_edit->>'duration_minutes')::INTEGER, duration_minutes) END,
  ...
  EXCEPTION WHEN SQLSTATE 'ZQ017' THEN
    PERFORM set_config('linkmia.rpc_writer', 'off', TRUE);
    INSERT INTO quote_verifications (verdict, mode, purpose, booking_id, identity_hash)
      VALUES (v_effective_verdict, v_mode, 'edit', p_booking_id, v_idhash);
    RETURN jsonb_build_object('outcome', CASE
      WHEN v_effective_verdict = 'expired' THEN 'quote_expired'
      ELSE 'quote_not_yet_valid' END);
...
BEGIN
  ...
  -- All three triggers must exist and be enabled in normal replication mode.
  ...
  IF v_count <> 3 THEN
    RAISE EXCEPTION '017 self-check: expected 3 enabled foundation triggers, found %', v_count;
  END IF;
...
BEGIN
  ...
  BEGIN
    ...
    -- actor class, and legacy `assigned` all bind the active slot.
    INSERT INTO bookings (
      ...
      'sedan', 10.00, 'assigned', 'website', TRUE
    ...
    IF v_result->>'outcome' <> 'created' OR NOT EXISTS (
    ...
    IF v_result->>'outcome' <> 'active_exists'
       OR (v_result->>'booking_id')::UUID <> v_booking_assigned THEN
    ...
    IF v_result->>'outcome' <> 'quote_not_yet_valid'
    ...
    IF v_result->>'outcome' <> 'created' OR NOT EXISTS (
      SELECT 1 FROM bookings WHERE id = v_booking_quote
        ...
        AND price_authority = 'client_observe' AND duration_minutes IS NULL
    ...
    IF v_result->>'outcome' <> 'idempotent' OR (v_result->>'booking_id')::UUID <> v_booking_quote THEN
    ...
    IF v_result->>'outcome' <> 'quote_consumed'
       OR (v_result->>'booking_id')::UUID <> v_booking_quote THEN
    ...
    IF v_result->>'outcome' <> 'conflict' OR EXISTS (
    ...
    IF v_result->>'outcome' <> 'updated' OR (v_result->>'details_version')::INTEGER <> 2 THEN
    ...
    IF v_result->>'outcome' <> 'idempotent' OR (v_result->>'details_version')::INTEGER <> 2 THEN
    ...
    v_result := accept_quote_edit(
      ...
      jsonb_build_object('airport_code','MIA','passengers',2,'duration_minutes',777)
    );
    IF v_result->>'outcome' <> 'updated' OR NOT EXISTS (
      SELECT 1 FROM bookings WHERE id = v_booking_legacy
        AND details_version = 3 AND assignment_epoch = 2
        AND duration_minutes IS NULL AND price = 20.00
    ...
    IF v_result->>'outcome' <> 'idempotent' THEN
    ...
    IF v_result->>'outcome' <> 'outdated_client' THEN
    ...
    IF v_result->>'outcome' <> 'quote_expired'
    ...
    IF v_result->>'outcome' <> 'updated'
       OR (v_result->>'details_version')::INTEGER <> 4 THEN
    ...
    DELETE FROM quote_acceptances
      WHERE booking_id IN (v_booking_legacy, v_booking_quote, v_booking_assigned);
    ...
    DELETE FROM bookings
      WHERE id IN (v_booking_legacy, v_booking_quote, v_booking_assigned);
